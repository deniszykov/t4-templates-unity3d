...
{
	[CustomEditor(typeof(TextTemplateImporter), true, isFallback = false)]
	internal class TextTemplateImporterInspector : ScriptedImporterEditor
	{
		...
		{
			if (this.assetTargets.Length == 1) this.InspectorGUI(this, this.assetTarget);
			this.ApplyRevertGUI();
		}
		...
		private void InspectorGUI(ScriptedImporterEditor editor, UnityObject assetTarget)
		{
			...
			{
				var settingsService = typeof(EditorApplication).Assembly.GetType("UnityEditor.SettingsService", false, true);
				var preferencesWindowType = typeof(EditorApplication).Assembly.GetType("UnityEditor.PreferencesWindow", false, true);
				var settingsWindowType = typeof(EditorApplication).Assembly.GetType("UnityEditor.SettingsWindow", false, true);
			...
			if ((generationTrigger & GenerationTriggers.AssetChanges) != 0)
			{
				...
				for (var i = 0; i < watchedAssetsProperty.arraySize; i++)
				{
					...
					if (watchedAsset != null)
						watchedAssetProperty.stringValue =
							AssetDatabase.GetAssetPath(EditorGUILayout.ObjectField(watchedAsset.GetType().Name, watchedAsset, typeof(UnityObject), false));
					else
