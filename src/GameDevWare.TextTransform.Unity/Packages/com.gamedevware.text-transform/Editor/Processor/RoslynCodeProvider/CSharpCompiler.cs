...
namespace GameDevWare.TextTransform.Editor.Processor.RoslynCodeProvider
{
	internal class CSharpCompiler : Compiler
	{
		...
		protected override void ProcessCompilerOutputLine(CompilerResults results, string line)
		{
			...
			var m = outputRegWithFileAndLine.Match(line);
			bool full;
			if (m.Success)
				full = true;
			else
			...
			if (m.Success)
			{
				...
				if (string.Compare(m.Groups[full ? 6 : 1].Value, "warning", StringComparison.OrdinalIgnoreCase) == 0) ce.IsWarning = true;
		...
		protected override void FixUpCompilerParameters(CompilerParameters options)
		{
			...
			{
				var noWarnStrings = new List<string>(5);
				noWarnStrings.AddRange(new[] { "1659", "1699", "1701" });
		...
		protected override string CmdArgsFromParameters(CompilerParameters parameters)
		{
			...
			var coreAssemblyFileName = parameters.CoreAssemblyFileName;
			if (string.IsNullOrWhiteSpace(coreAssemblyFileName)) coreAssemblyFileName = coreAssembly;
			...
			if (systemRuntimeAssemblyPath != null) allArgsBuilder.Append($"/R:\"{systemRuntimeAssemblyPath}\" ");

			foreach (var s in parameters.ReferencedAssemblies)
			{
...
#if !FEATURE_PAL
			if (parameters.Win32Resource != null) allArgsBuilder.Append("/win32res:\"" + parameters.Win32Resource + "\" ");
#endif // !FEATURE_PAL

			foreach (var s in parameters.EmbeddedResources)
			{
			...
			foreach (var s in parameters.LinkedResources)
			{
			...
			if (parameters.WarningLevel >= 0) allArgsBuilder.Append("/w:" + parameters.WarningLevel + " ");

			if (parameters.CompilerOptions != null) allArgsBuilder.Append(parameters.CompilerOptions + " ");

			var allArgs = allArgsBuilder.ToString();
			return allArgs;
		}
	}
}
